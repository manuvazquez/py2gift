...
__all__ = ['markdown_header', 'html_question_to_markdown', 'numerical_to_markdown', 'multiple_choice_to_markdown',
           'SvgToMarkdown', 'settings_to_markdown']
...
# Cell
class SvgToMarkdown(gift_wrapper.question.QuestionDecorator):
    """
    Decorator to reformat svg files for including them in markdown strings.
    """

    def __init__(self, decorated: Union[gift_wrapper.question.HtmlQuestion, gift_wrapper.question.QuestionDecorator]):
        ...
        self.pre_processing_functions.append(functools.partial(
            self.transform_files, pattern=gift_wrapper.parsing.svg_file, process_match=process_match,
            replacement=r'![](' + r'\1)'))

    def to_markdown(self) -> str:

        markdown = self._decorated.to_markdown()

        for f in self.pre_processing_functions:
...
def settings_to_markdown(settings: dict) -> str:
    ...
    **Returns**
    ...
    question = SvgToMarkdown(gift_wrapper.question.TexToSvg(cls(**settings)))
